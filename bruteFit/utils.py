def launch_proc_viewer():
    ...
    class ProcViewer(QWidget):
        def __init__(self):
            ...
            self.pathlength_spin = QDoubleSpinBox();         self.pathlength_spin.setRange(0, 1e12); self.pathlength_spin.setDecimals(10); self.pathlength_spin.setValue(1.0)
            self._labeled(top_row, "Pathlength:", self.pathlength_spin)
            self.conc_spin = QDoubleSpinBox();               self.conc_spin.setRange(0, 1e12); self.conc_spin.setDecimals(10); self.conc_spin.setValue(2.0)
            self._labeled(top_row, "Concentration:", self.conc_spin)
            self.field_spin = QDoubleSpinBox();              self.field_spin.setRange(-1e12, 1e12); self.field_spin.setDecimals(10); self.field_spin.setValue(3.0)
